...
var origin = new location(0,200,200);
var triplog = [];
triplog[0]=origin;
triplog[1]=origin;
var count = 0;
...
//Santa goes on his journey.
while(count<directions.length){
  triplog[count+2]=move(triplog[count],currentdirection(directions[count]));
  count=count+2;
};


//Robo-Santa starts his journey.
count=1;
while(count<directions.length){
  triplog[count+2]=move(triplog[count],currentdirection(directions[count]));
  count=count+2;
...
//Counts all of the unique houses in the presentmatrix
count=0;
while(count<directions.length){
